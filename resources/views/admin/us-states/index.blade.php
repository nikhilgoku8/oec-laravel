            ...
            <div class="page-header my_style">
                ...
                <div class="right_section">
                    <div class="purple_hollow_btn">
                        <a href="{{ route('us-states.create'); }}">Add New</a>
                    </div>
    ...
    <div class="row">
        <div class="fourth_row">
            ...
            <div class="my_panel">
                ...
                <div class="upper_sec">
                    <div class="left_section">
                        <div class="title">US States Data</div>
                    ...
                    <table>
                        <tbody>
                            <tr>
                                <th>US State</th>
                                <th>Abbrevation</th>
                                <th>Sales <br> Representatives</th>
                            ...
                            @if($result->isNotEmpty())
                                @foreach ($result as $row)
                                    <tr>
                                        ...
                                        <td>{{ $row->abbr }}</td>
                                        <td>
                                            @if(!empty($row->salesRepresentatives) && count($row->salesRepresentatives) > 0)
                                                @foreach($row->salesRepresentatives as $rep)
                                                    @if($loop->iteration != 1)
                                                         , 
                                                    @endif
                                                    {{ $rep->rep_name }}
                                        ...
                                        <td class="action">
                                            <a href="{{ route('us-states.edit', $row->id) }}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                            <span class="checkbox">
  ...
  $("#delete_records").on('click',(function(e){
    ...
    if(dataID.length == 0){
        ...
        if (confirm('Are you sure you want to delete these records?')) {
            $.ajax({
                type: "POST",
                url: "{{ route('us-states.bulk-delete') }}",
